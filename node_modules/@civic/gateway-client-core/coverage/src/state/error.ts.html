...
<head>
    <title>Code coverage report for src/state/error.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/state</a> error.ts</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { DEFAULT_MAX_CLIENT_SENDS_RETRIES } from '../utils/defaults';
...
export const handleChainError = (state: GatewayCoreState): InternalGatewayStatus =&gt; {
  if (
    ...
    state.inputs.civicPass.received<span class="branch-0 cbranch-no" title="branch not covered" >?.a</span>ction === CivicPassMessageAction.CHAIN_ERROR
  ) {
    const isRefreshFlow = !!state.inputs.gatewayToken<span class="branch-0 cbranch-no" title="branch not covered" >?.r</span>eceived;
&nbsp;
...
&nbsp;
    ...
    if (
      ...
      (state.internal.chainTransaction.attempts || <span class="branch-1 cbranch-no" title="branch not covered" >0)</span> &lt;=
      ...
      if (isRefreshFlow) {
        return payerIsWalletOwner(state)
          ...
          : <span class="branch-1 cbranch-no" title="branch not covered" >ExtendedGatewayStatus.REFRESH_AWAITING_THIRD_PARTY_TRANSACTION_SEND;</span>
      }
&nbsp;
      return payerIsWalletOwner(state)
        ? ExtendedGatewayStatus.ISSUANCE_AWAITING_TRANSACTION_SEND
        : <span class="branch-1 cbranch-no" title="branch not covered" >ExtendedGatewayStatus.ISSUANCE_AWAITING_THIRD_PARTY_TRANSACTION_SEND;</span>
    }
&nbsp;
...
&nbsp;
  ...
  return state.internal<span class="branch-0 cbranch-no" title="branch not covered" >?.s</span>tatus as InternalGatewayStatus;
};
&nbsp;
export const handleGatekeeperError = (state: GatewayCoreState): InternalGatewayStatus =&gt; {
  ...
  if (
    state.inputs.civicPass.received?.event === CivicPassMessageEventResult.FAILURE &amp;&amp;
    state.inputs.civicPass.received<span class="branch-0 cbranch-no" title="branch not covered" >?.a</span>ction === CivicPassMessageAction.ERROR
  ) {
    return isRefreshFlow ? ExtendedGatewayStatus.RESTART_REFRESH : ExtendedGatewayStatus.RESTART;
  }
  return state.internal<span class="branch-0 cbranch-no" title="branch not covered" >?.s</span>tatus as InternalGatewayStatus;
