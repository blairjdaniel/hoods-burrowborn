...
<head>
    <title>Code coverage report for src/orchestrate.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">src</a> orchestrate.ts</h1>
        <div class='clearfix'>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>169/180</span>
            </div>
...
&nbsp;
  constructor(
    ...
    if (!this.chainImplementation || !inputs.wallet<span class="branch-0 cbranch-no" title="branch not covered" >?.a</span>ddress || !inputs.stage) {
...
&nbsp;
    ...
    const chainNetwork = this.chainImplementation<span class="branch-0 cbranch-no" title="branch not covered" >?.c</span>hainDetails?.chainNetwork;
&nbsp;
    this.gatekeeperClient = new GatekeeperClient({
      abortController: this.abortController,
      baseUrl: inputs.gatekeeperConfig<span class="branch-0 cbranch-no" title="branch not covered" >?.b</span>aseUrl, // if not set, this will be derived from the stage in gatekeeperClient.ts
      ...
      headers: {
        ...(inputs.gatekeeperConfig<span class="branch-0 cbranch-no" title="branch not covered" >?.h</span>eaders || <span class="branch-1 cbranch-no" title="branch not covered" >{})</span>,
      ...
      walletAddress: inputs.wallet<span class="branch-0 cbranch-no" title="branch not covered" >?.a</span>ddress,
      chainType: this.chainImplementation<span class="branch-0 cbranch-no" title="branch not covered" >?.c</span>hainDetails?.chainType,
    });
&nbsp;
...
&nbsp;
  private get partnerReview() {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!this._partnerReview) {
...
&nbsp;
  async fetchGatekeeperNetworkData() {
    ...
    return getGatekeeperNetworkData(this.inputs.stage || <span class="branch-1 cbranch-no" title="branch not covered" >'prod',</span> this.inputs.gatekeeperNetwork || <span class="branch-1 cbranch-no" title="branch not covered" >'')</span>
      .then(<span class="fstat-no" title="function not covered" >(r</span>esponse) =&gt; {
<span class="cstat-no" title="statement not covered" >        this.log.debug('getGatekeeperNetworkData', response);</span>
<span class="cstat-no" title="statement not covered" >        if (this.abortController.signal.aborted) <span class="cstat-no" title="statement not covered" >return;</span></span>
<span class="cstat-no" title="statement not covered" >        this.gatewayCoreStore.setState(<span class="fstat-no" title="function not covered" >(s</span>tate) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >          state.inputs.gatekeeperNetworkData.status = InputStatus.COMPLETE;</span>
<span class="cstat-no" title="statement not covered" >          state.inputs.gatekeeperNetworkData.received = response;</span>
        });
      })
      .catch((error) =&gt; {
    ...
    const retrieveGKRecord = async () =&gt; {
      ...
      return this.gatekeeperClient
        ...
        .then((response) =&gt; {
          ...
          <span class="missing-if-branch" title="if path not taken" >I</span>if (this.abortController.signal.aborted) <span class="cstat-no" title="statement not covered" >return;</span>
          this.gatewayCoreStore.setState((state) =&gt; {
  ...
  onInternalStatusChange(status: InternalGatewayStatus | null, previousStatus?: InternalGatewayStatus | null) {
    if (status === GatewayStatus.ERROR) {
      ...
      if (tokenNoGatekeeperRecordError(fullState)) {
        this.gatewayCoreStore.setState((state) =&gt; {
          <span class="missing-if-branch" title="if path not taken" >I</span>if (!state.internal.errors) {
<span class="cstat-no" title="statement not covered" >            state.internal.errors = {};</span>
          }
    ...
    if (status === GatewayStatus.ACTIVE) {
      ...
      <span class="missing-if-branch" title="else path not taken" >E</span>if (previousStatus !== GatewayStatus.ACTIVE) {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (status === ExtendedGatewayStatus.ISSUANCE_RESTART_DATA_COLLECTION) {
<span class="cstat-no" title="statement not covered" >      return this.issuance.restartDataCollection();</span>
    }
...
<span class="cstat-no" title="statement not covered" >            this.log.warn('No result from remote sign event');</span>
          }
...
<span class="cstat-no" title="statement not covered" >          this.gatewayCoreStore.setState(<span class="fstat-no" title="function not covered" >(d</span>raft) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >            draft.inputs.civicSign.error = error;</span>
<span class="cstat-no" title="statement not covered" >            draft.inputs.civicSign.status = InputStatus.ERROR;</span>
          });
        });
    });
&nbsp;
