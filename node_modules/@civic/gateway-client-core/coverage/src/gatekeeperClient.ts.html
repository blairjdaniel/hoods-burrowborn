...
<head>
    <title>Code coverage report for src/gatekeeperClient.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">src</a> gatekeeperClient.ts</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">92.7% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>127/137</span>
            </div>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { default as fetchBuilder } from 'fetch-retry';
...
&nbsp;
export const isGkApiStatusTokenCreated = <span class="fstat-no" title="function not covered" >(c</span>ode: number): boolean =&gt; <span class="cstat-no" title="statement not covered" >code === GatekeeperAPIStatus.ISSUED;</span>
...
&nbsp;
  constructor(public initConfig: GatekeeperClientConfig, flowId?: string) {
    ...
    this.baseUrl = initConfig.baseUrl || <span class="branch-1 cbranch-no" title="branch not covered" >getGatekeeperEndpoint(initConfig.stage, initConfig.chainType);</span>
    ...
    this.defaultRetryParams = {
      ...
      retryOn: async (attempt: number, error: Error | null, response: Response | null): Promise&lt;boolean&gt; =&gt; {
        ...
        if (response &amp;&amp; response.status &gt;= 500) {
          <span class="missing-if-branch" title="if path not taken" >I</span>if (body<span class="branch-0 cbranch-no" title="branch not covered" >?.m</span>essage<span class="branch-1 cbranch-no" title="branch not covered" >?.includes('NO_AUTO_RETRY'))</span> {
...
&nbsp;
  public setFlowId(flowId: string): void {
    this.headers = objectToURLParams({ ...(this.headers || <span class="branch-1 cbranch-no" title="branch not covered" >{})</span>, 'x-civic-flowid': flowId });
  }
&nbsp;
...
&nbsp;
  private continueIfNotAborted&lt;T&gt;(fn: () =&gt; T): T | null {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!this.abortController.signal.aborted) {
      return fn();
    }
<span class="cstat-no" title="statement not covered" >    return null;</span>
  }
&nbsp;
  private urlForWallet(walletAddress: string = <span class="branch-0 cbranch-no" title="branch not covered" >this.initConfig.walletAddress)</span>: string {
...
&nbsp;
  public async getGatekeeperRecordWithPayload(
    ...
    return this.fetchWithRetry(this.urlForWallet(walletAddress), {
      ...
      .catch((error) =&gt; {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (error.name === 'AbortError') {
<span class="cstat-no" title="statement not covered" >          logDebug('error due to abort controller signal aborted');</span>
<span class="cstat-no" title="statement not covered" >          return null;</span>
        }
        throw error;
      });
...
&nbsp;
  ...
  public async fetchFreshTransaction({ payer }: TokenRequest): Promise&lt;RequestTokenRefreshResponse | null&gt; {
    ...
    this.addQueryParams(url);
    logDebug(
      `fetchFreshTransaction: ${url.toString()}${this.queryParams<span class="branch-0 cbranch-no" title="branch not covered" >?.g</span>atekeeperNetworkAddress<span class="branch-0 cbranch-no" title="branch not covered" >?.s</span>ubstring(
        0,
        6
...
<span class="cstat-no" title="statement not covered" >        logError('fetchFreshTransaction', error);</span>
