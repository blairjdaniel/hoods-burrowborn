...
<head>
    <title>Code coverage report for src/issuance.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">src</a> issuance.ts</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { GatekeeperClient } from './gatekeeperClient';
...
&nbsp;
  ...
  async sendTransaction() {
    ...
    if (!fullState.inputs.gatekeeperRecord.received<span class="branch-0 cbranch-no" title="branch not covered" >?.t</span>ransaction) {
...
&nbsp;
    ...
    this.gatewayCoreStore.setState((draft) =&gt; {
      ...
      <span class="missing-if-branch" title="else path not taken" >E</span>if (draft.internal.chainTransaction.attempts !== undefined) {
    ...
    this.chainImplementation
      ...
      .then((txId) =&gt; {
        ...
        this.gatewayCoreStore.setState((state) =&gt; {
          <span class="missing-if-branch" title="if path not taken" >I</span>if (!state.internal.chainTransaction) {
<span class="cstat-no" title="statement not covered" >            state.internal.chainTransaction = {};</span>
          }
&nbsp;
...
&nbsp;
      ...
      .catch((error) =&gt; {
        this.gatewayCoreStore.setState((draft) =&gt; {
          <span class="missing-if-branch" title="if path not taken" >I</span>if (!draft.internal.chainTransaction) {
<span class="cstat-no" title="statement not covered" >            draft.internal.chainTransaction = {};</span>
          }
...
&nbsp;
  async gatekeeperRequest(payer: string | undefined, fullState: GatewayCoreState) {
    ...
    this.currentPayload = fullState.inputs.civicPass.received<span class="branch-0 cbranch-no" title="branch not covered" >?.p</span>ayload;
    return this.gatekeeperClient.requestGatewayTokenFromGatekeeper({
      payload: fullState.inputs.civicPass.received<span class="branch-0 cbranch-no" title="branch not covered" >?.p</span>ayload,
      payer,
    } as TokenRequest);
  }
&nbsp;
...
&nbsp;
    this.timers.expectOnChainToken = setTimeout(() =&gt; {
      this.gatewayCoreStore.setState((state) =&gt; {
        <span class="missing-if-branch" title="else path not taken" >E</span>if (!this.statusHasValidToken()) {
...
&nbsp;
  ...
  startRefreshCountdownTimer() {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!gatewayToken<span class="branch-0 cbranch-no" title="branch not covered" >?.e</span>xpiryTime) {
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
&nbsp;
    this.abortRefreshCountdownTimer();
    const refreshIntervalDuration = getTokenRefreshIntervalMilliseconds(gatewayToken.expiryTime, 0);
    this.timers.refreshCountdownTimer = setTimeout(<span class="fstat-no" title="function not covered" >() =</span>&gt; {
<span class="cstat-no" title="statement not covered" >      this.abortRefreshCountdownTimer();</span>
