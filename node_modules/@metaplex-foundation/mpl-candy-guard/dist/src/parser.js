...
function deserializeGuardSet(buffer) {
    ...
    if (solPaymentEnabled) {
        const [solPayment] = generated_1.solPaymentBeet.deserialize(buffer, cursor);
        data.solPayment = solPayment;
        cursor += GUARDS_SIZE.solPayment;
    }
    if (tokenPaymentEnabled) {
        const [tokenPayment] = generated_1.tokenPaymentBeet.deserialize(buffer, cursor);
    ...
    if (endDateEnabled) {
        const [endDate] = generated_1.endDateBeet.deserialize(buffer, cursor);
    ...
    if (redeemedAmountEnabled) {
        const [redeemedAmount] = generated_1.redeemedAmountBeet.deserialize(buffer, cursor);
        data.redeemedAmount = redeemedAmount;
        cursor += GUARDS_SIZE.redeemedAmount;
    }
    if (addressGateEnabled) {
        const [addressGate] = generated_1.addressGateBeet.deserialize(buffer, cursor);
        data.addressGate = addressGate;
        cursor += GUARDS_SIZE.addressGate;
    }
    if (nftGateEnabled) {
        const [nftGate] = generated_1.nftGateBeet.deserialize(buffer, cursor);
        data.nftGate = nftGate;
        cursor += GUARDS_SIZE.nftGate;
    }
    if (nftBurnEnabled) {
        const [nftBurn] = generated_1.nftBurnBeet.deserialize(buffer, cursor);
        data.nftBurn = nftBurn;
        cursor += GUARDS_SIZE.nftBurn;
    }
    if (tokenBurnEnabled) {
        const [tokenBurn] = generated_1.tokenBurnBeet.deserialize(buffer, cursor);
    ...
    if (token2022PaymentEnabled) {
        const [token2022Payment] = generated_1.token2022PaymentBeet.deserialize(buffer, cursor);
    ...
    return {
        guardSet: {
            ...
            token2022Payment: (_w = data.token2022Payment) !== null && _w !== void 0 ? _w : null,
        },
        offset: cursor,
    };
}
function serializeGuardSet(buffer, offset, guardSet) {
    ...
    if (guardSet.solPayment) {
        generated_1.solPaymentBeet.write(buffer, offset, guardSet.solPayment);
    ...
    index++;
    if (guardSet.endDate) {
        generated_1.endDateBeet.write(buffer, offset, guardSet.endDate);
    ...
    if (guardSet.redeemedAmount) {
        generated_1.redeemedAmountBeet.write(buffer, offset, guardSet.redeemedAmount);
        offset += GUARDS_SIZE.redeemedAmount;
        features |= 1 << index;
    }
    index++;
    if (guardSet.addressGate) {
        generated_1.addressGateBeet.write(buffer, offset, guardSet.addressGate);
        offset += GUARDS_SIZE.addressGate;
        features |= 1 << index;
    }
    index++;
    if (guardSet.nftGate) {
        generated_1.nftGateBeet.write(buffer, offset, guardSet.nftGate);
        offset += GUARDS_SIZE.nftGate;
        features |= 1 << index;
    }
    index++;
    if (guardSet.nftBurn) {
        generated_1.nftBurnBeet.write(buffer, offset, guardSet.nftBurn);
        offset += GUARDS_SIZE.nftBurn;
        features |= 1 << index;
    }
    index++;
    if (guardSet.tokenBurn) {
        generated_1.tokenBurnBeet.write(buffer, offset, guardSet.tokenBurn);
    ...
    if (guardSet.token2022Payment) {
        generated_1.token2022PaymentBeet.write(buffer, offset, guardSet.token2022Payment);
