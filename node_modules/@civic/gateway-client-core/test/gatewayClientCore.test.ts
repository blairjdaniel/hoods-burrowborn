...
describe('GatewayClientCore tests', () => {
  ...
  context('reset', () => {
    let initialiseStub: sinon.SinonStub;

    beforeEach(() => {
      initialiseStub = sandbox.stub(Orchestrator.prototype, 'initialise');
    });

    it('should call inititialize on reset', () => {
      coreInput = testHelpers.getCoreInputs();
      const instance = new GatewayClientCore(coreInput);

      instance.reset();

      // should call initialise twice, once on creation and once on reset
      expect(initialiseStub).to.have.been.calledTwice;
    });
  });
});
