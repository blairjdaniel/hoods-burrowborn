...
describe('computeFlowParameters', () => {
  ...
  context('feeAmountUSDCents', () => {
    context('with feeAmountUSDCents set in the GK record payload', () => {
      beforeEach(() => {
        state = R.assocPath<string, GatewayCoreState>(
          ['inputs', 'gatekeeperRecord', 'received', 'payload', 'feeAmountUSDCents'],
          '111'
        )(state);
      });
      it('should use the value from the GK record received payload', () => {
        expect(computeFlowParameters(state, GatewayStatus.ACTIVE)?.feeAmountUSDCents).to.equal('111');
      });
    });
    context('with feeAmountUSDCents set in the GK record', () => {
      beforeEach(() => {
        state = R.assocPath<string, GatewayCoreState>(
          ['inputs', 'gatekeeperRecord', 'received', 'feeAmountUSDCents'],
          '222'
        )(state);
      });
      it('should use the value from the GK record received', () => {
        expect(computeFlowParameters(state, GatewayStatus.ACTIVE)?.feeAmountUSDCents).to.equal('222');
      });
    });
    context('with no feeAmountUSDCents on the GK record and fees set on the output flowParameters', () => {
      beforeEach(() => {
        state = R.assocPath<string, GatewayCoreState>(['output', 'flowParameters', 'feeAmountUSDCents'], '333')(state);
      });
      it('should use the value from the outputs flowParameters', () => {
        expect(computeFlowParameters(state, GatewayStatus.ACTIVE)?.feeAmountUSDCents).to.equal('333');
      });
    });
    context('with feeAmountUSDCents on the GK record and on the output flowParameters', () => {
      beforeEach(() => {
        state = R.assocPath<string, GatewayCoreState>(['output', 'flowParameters', 'feeAmountUSDCents'], '444')(state);
        state = R.assocPath<string, GatewayCoreState>(
          ['inputs', 'gatekeeperRecord', 'received', 'feeAmountUSDCents'],
          '555'
        )(state);
      });
      it('should use the value from the GK record', () => {
        expect(computeFlowParameters(state, GatewayStatus.ACTIVE)?.feeAmountUSDCents).to.equal('555');
      });
    });
    context('with no feeAmountUSDCents on the GK record and not fees set on the output flowParameters', () => {
      it('should use undefined', () => {
        expect(computeFlowParameters(state, GatewayStatus.ACTIVE)?.feeAmountUSDCents).to.equal(undefined);
      });
    });
  });
});
