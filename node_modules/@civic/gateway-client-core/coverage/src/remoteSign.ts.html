...
<head>
    <title>Code coverage report for src/remoteSign.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">src</a> remoteSign.ts</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { prefixLogger } from './logger';
...
&nbsp;
  ...
  if (message.request === CivicSignEventTypeRequest.REQUEST_PUBLIC_KEY) {
    remoteSignerInst<span class="branch-0 cbranch-no" title="branch not covered" >?.s</span>endPublicKey(wallet.address);
    return wallet.address;
  }
  if (message.request === CivicSignEventTypeRequest.REQUEST_DID) {
    remoteSignerInst<span class="branch-0 cbranch-no" title="branch not covered" >?.s</span>endDid(ownerDids<span class="branch-0 cbranch-no" title="branch not covered" >?.[0</span>]);
    return ownerDids<span class="branch-0 cbranch-no" title="branch not covered" >?.[0</span>];
  }
  if (message.request === CivicSignEventTypeRequest.REQUEST_SIGNED_PROOF) {
    ...
    remoteSignerInst<span class="branch-0 cbranch-no" title="branch not covered" >?.s</span>endSignedProof(powoResponse.proof, powoResponse.signatureMethod);
    return { proof: powoResponse.proof, signatureMethod: powoResponse.signatureMethod };
  }
  <span class="missing-if-branch" title="else path not taken" >E</span>if (message.request === CivicSignEventTypeRequest.REQUEST_SIGNED_MESSAGE) {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (message.payload) {
      ...
      // TODO send back a remote sign error: this requires the iframe to handle this
      remoteSignerInst<span class="branch-0 cbranch-no" title="branch not covered" >?.s</span>endSignedMessage(Buffer.from(proveOwnershipResponse.proof));
