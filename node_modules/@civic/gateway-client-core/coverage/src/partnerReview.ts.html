...
<head>
    <title>Code coverage report for src/partnerReview.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">src</a> partnerReview.ts</h1>
        <div class='clearfix'>
...
&nbsp;
  ...
  async pollForReviewFinished() {
    ...
    const callGatekeeperForReviewFinished = async () =&gt; {
      <span class="missing-if-branch" title="if path not taken" >I</span>if (this.abortController<span class="branch-0 cbranch-no" title="branch not covered" >?.s</span>ignal.aborted) {
<span class="cstat-no" title="statement not covered" >        clearInterval(this.timers.pollForReviewFinishedTimer);</span>
<span class="cstat-no" title="statement not covered" >        return;</span>
      }
      this.gatekeeperClient.getGatekeeperRecordWithPayload().then((record: GatekeeperRecordResponse) =&gt; {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (!record || record<span class="branch-0 cbranch-no" title="branch not covered" >?.s</span>tate === undefined || record<span class="branch-0 cbranch-no" title="branch not covered" >?.s</span>tate === GatekeeperAPIStatus.SERVER_FAILURE) {
<span class="cstat-no" title="statement not covered" >          this.log.error('Error polling gatekeeper, no state returned', record);</span>
<span class="cstat-no" title="statement not covered" >          clearInterval(this.timers.pollForReviewFinishedTimer);</span>
<span class="cstat-no" title="statement not covered" >          throw new Error('Error polling gatekeeper, no state returned');</span>
        }
